...
export class Schema {
  ...
  init (config = {}, schemaTweaks) {
    ...
    const argOptions = processArgs(workingConfig)
    const configFile = (() => {
      switch (true) {
        case fs.existsSync(argOptions.configFile):
          return argOptions.configFile
        case fs.existsSync(workingConfig.configFile):
          return workingConfig.configFile
        case fs.existsSync(`.config/${workingConfig.configFile}`):
          return `.config/${workingConfig.configFile}`
        case fs.existsSync(`~/.config/${workingConfig.configFile}`):
          return `.config/${workingConfig.configFile}`
        default:
          return {}
    ...
    const hierarchy = []
      .push(workingConfig.defaults)
      .push(workingConfig.defaults)
      .push(workingConfig.configFile ? configFile : {})
      .push(config)
      .push(workingConfig.processEnv ? processEnv(workingConfig) : {})
      .push(workingConfig.processSTDIN ? argOptions : {})

    return Object.assign(workingConfig, ...hierarchy)
  }
}
