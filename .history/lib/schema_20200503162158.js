...
export class Schema {
  constructor (schema) {
    this.validateSync(schema, masterSchema, (err, schema) => {
      if (err) throw err
    })
  }
  ...
  validate (config, schema, callback) { return Schema.validateSync(config, schema, callback) }
   ...
   */
  validateSync (config, schema, callback) {
    try {
